...
(function () {
    ...
    function processLogin(supportedFeatures, context) {
        ...
        fetch(crealApiBaseUrl + '/v1/messenger/login', {
            ...
            me.creal.frontend.core.init({
                messengerFeatures: supportedFeatures,
    ...
    function handleLogin() {
        ...
        MessengerExtensions.getSupportedFeatures(function (result) {
            var supportedFeatures = result.supported_features;
            if (supportedFeatures.indexOf('context') >= 0) {
                MessengerExtensions.getContext(crealAppId, function (context) {
                    processLogin(supportedFeatures, context);
                }, function (code, message) {
                    showError(code, message);
                });
            } else {
